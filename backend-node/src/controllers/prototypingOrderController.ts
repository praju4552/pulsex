...
export async function createPrototypingOrder(req: Request, res: Response) {
  try {
    const {
      ...
      billingName, billingStreetAddress, billingApartment, billingCity, billingState, billingZip, billingCountry,
      gstNumber,
...
export async function downloadPrototypingDocument(req: Request, res: Response) {
  try {
    ...
    await generateOrderPDF(res, {
      ...
      // Billing
      billingName: (order as any).billingName || undefined,
      billingStreetAddress: (order as any).billingStreetAddress || undefined,
      billingApartment: (order as any).billingApartment || undefined,
      billingCity: (order as any).billingCity || undefined,
      billingState: (order as any).billingState || undefined,
      billingZip: (order as any).billingZip || undefined,
      billingCountry: (order as any).billingCountry || undefined,
      gstNumber: (order as any).gstNumber || undefined,
